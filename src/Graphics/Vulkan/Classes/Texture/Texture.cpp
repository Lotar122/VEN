...
Texture::Texture(const std::string& path, AssetUsage _assetUsage, Engine* _engine, uint8_t desiredChannels) : Asset(_assetUsage, _engine)
{
...
Texture::Texture(const char* data, size_t _width, size_t _height, uint8_t _channels, AssetUsage _assetUsage, Engine* _engine, uint8_t desiredChannels) : Asset(_assetUsage, _engine)
{
    assert(_engine != nullptr);
    engine = _engine;
...
vk::DescriptorSetLayoutBinding  Texture::getDescriptorSetLayoutBinding(vk::ShaderStageFlagBits shaderStage, uint32_t binding)
{
...
void Texture::update(const std::vector<unsigned char>& _data)
{
...
void Texture::update(const unsigned char* _data, size_t _size)
{
    assert(_size == size);
